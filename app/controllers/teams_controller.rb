class TeamsController < ApplicationController
  ...
  def create
    ...
    simulation = Simulation.get_current_simulation()
    Round.all.each do |round|
      TeamRound.initialize(@team,round,simulation)
    end
  ...
  def sign_up
    my_params = team_params
    # my_params[:simulation_id] = current_simulation.id
    @team = Team.new(my_params)
    @team.save!
  ...
  def export
    ...
    csv_string = CSV.generate do |csv|
      ...
      team_round.decisions.order(:input_item_id).each do |d|
        csv << [d.field_name,d.field_value]
      end
    end
    filename = team.name + "_" + transliterate(team_round.round.name) + "_decisions"
  ...
  def team_params
    params.require(:team).permit(:name, :password, :simulation_id , :team_rounds_attributes => [:is_finished, :id, :report , :debrief])
  end
